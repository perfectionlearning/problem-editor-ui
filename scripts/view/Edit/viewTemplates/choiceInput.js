...
;(function() {
	...
	//=======================================================
	//=======================================================
	app.ChoiceListView = app.PEView.extend({
		header: 'Answer',
		compare: app.objectCompare,
		field: 'a',

		colHeaders: ['Choices', 'Answer'],
		...
		valueFromModel: function(model) {
			var choices = (model && model.get('choices'));
			if (!choices || choices.length < 1)
				choices = [''];

			var answers = (model && model.get(this.field)) || '';
			return makeList(choices, answers);
		},
		...
		value: function() {
			...
			for (var i = 0, len = choices.length; i < len; i++)
			{
				...
				data.push([entry, answers[i].checked])
			}
	...
	//=======================================================
	function makeList(choices, answers)
	{
		var out = [];
		var ansList = makeIntList(answers);

		for (var i = 0; i < choices.length; i++)
			out.push([choices[i], ansList.indexOf(i) !== -1]);
