...
var $ = jQuery,
  ...
  min = Math.min,
  max = Math.max;
  var preventInsertions = false,  // @DG+
  insertionCallback;  // @DG+
...
function createRoot(jQ, root, textbox, editable) {
  ...
  function textInput() {
    ...
    if (text && !preventInsertions) {
      textarea.val('');
      for (var i = 0; i < text.length; i += 1) {
        cursor.parent.textInput(text.charAt(i));
      }
      if(insertionCallback) insertionCallback();
    }
    else {
      if (cursor.selection || textareaSelectionTimeout !== undefined)
        setTextareaSelection();
...
//  bind(VanillaSymbol,'\\wedge ','&and;'); // @DG-
  bind(VanillaSymbol,'\\wedge ',' and '); // @DG+ -- Use " and " instead of the wedge symbol
  ...
  case 'write':
    if (arguments.length > 1)
      return this.each(function() {
        ...
        if (cursor) {
          cursor.write(latex);    // @DG+
//@DG-          cursor.writeLatex(latex);
          block.blur();
        }
      });
    break;  // @DG+
// @DG++
  case 'negate':  // @DG+: Ugly hack to support KI-like negative toggle
    var data = $(this).data(jQueryDataKey),
      block = data && data.block,
      cursor = block && block.cursor;
    if (cursor && cursor.prev.cmd === '-')
      cursor.backspace();
    else if (cursor)
      cursor.write('-');
    return;
  case 'insertDisable':
    preventInsertions = true;
    break;
  case 'insertEnable':
    preventInsertions = false;
    break;
// @DG_end
  default:
