...
(function($){if(typeof $.fn.btnAddRow== "undefined"){
	...
	ExpandableTable.prototype.live=function(){
		if (!this.goLive){
			var t=this;
			this.update();
			$(document).on("click", ".addRow"+this.seed, function(){
				var newRow=t.addRow();
			});
			$(document).on("click", ".delRow"+this.seed, function(){
				var oj=$(this).closest("."+t.cloneClass),
				o=oj.clone();

				// DG: Add a pre-add callback as a partner to the post-add callback
				if(t.preDelCallBack && $.isFunction(t.preDelCallBack))
					t.preDelCallBack(o);
			...
			$(document).on("keyup", ".autoAdd"+this.seed, function(){
	...
	ExpandableTable.prototype.addRow=function(){
		// DG: Add a pre-add callback as a partner to the post-add callback
		if(this.preAddCallBack && $.isFunction(this.preAddCallBack))
			this.preAddCallBack();
		...
		if(!this.maxRow || (this.maxRow && $("."+this.cloneClass).size()<this.maxRow)){
			...
			//-----------------------------------------------------
			// Perform some cleanup of the values in the cloned row
			newRow.find("input:text").val("");
			newRow.find("textarea").text("");
//			newRow.find("div[contenteditable='true']").text("");	// DG: Clear new contenteditable divs as well
			newRow.find("div.clearme").text("");	// DG: Clear new contenteditable divs as well
			newRow.find('input[type="radio"],input[type="checkbox"]').prop('checked', false);	// DG: Clear radio/checkbox selection
			//-----------------------------------------------------
	...
	$.fn.btnAddRow=$.fn.tableAutoAddRow=function(options,func){
		...
		this.each(function(){
			...
			if(tbl.size()>0){
				...
				etbl.preAddCallBack=options.preAddCallBack;
			};
		});
		...
		this.each(function(){
			...
			if(tbl.size()>0){
				...
				if($(this).data(className)) {
					...
					etbl.preDelCallBack=options.preDelCallBack;
				}
